...
`tutorials/deplot_chart_colab.ipynb` (`E2E`, **standalone** carrier) is **Release-grade** at source revision `ddf0cc4`
and notebook blob `90d9093cced4`, committed at `dd5724e`. That exact blob executed top-to-bottom in a fresh Kaggle
Tesla T4 runtime on 2026-09-26. Unit tests, JSON validation, code-cell compilation, the generator parity checks and
...
| 2026-09-26 (07:06–08:30) | `dd5724e` / `90d9093cced4` (`NOTEBOOK_SOURCE.repository_revision` = `ddf0cc4`, the source revision the notebook was generated at; `ddf0cc4..dd5724e` changes the generated notebook and release-status documentation) | Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-deplot-chart` v4), serial suite; exact committed blob fetched and Git-blob verified, executed verbatim in a fresh interpreter with a `google.colab` shim and no repository checkout; Hugging Face cache clean at start; image `gcr.io/kaggle-gpu-images/python@sha256:37c64f7dd9c54116ecd1bcc88817c5469b88387388fade02bfa8bf3fc647d461`, Python 3.12.13, Tesla T4 15360 MiB, driver 580.159.04; after inline pins: torch 2.14.0+cu130 (CUDA 13.0), transformers 4.57.6, device `cuda:0`, float32 | Default sample path (`USE_BYOD = False`, all form defaults), all sections. The 8-file DePlot snapshot and pinned SynthChartNet shard were fetched into an empty cache and digest-verified; 621 staged files / 1,670,135,453 bytes. Corpus split 360 / 80 / 160 charts; test chart types bar 72, line 1, pie 73, stacked-bar 14. Frozen test: cell accuracy 0.160, RNSS 0.448, exact-table match 0.006, truncation 0.050; empty/header-only/medoid cell-accuracy baselines 0.000 / 0.007 / 0.037. Adaptation trained 18,879,744 of 282,285,696 parameters for three epochs, best epoch 2 by validation cell accuracy (0.123 frozen → 0.162), 2290.3 s. Adapted test: cell accuracy 0.181 (+0.021), RNSS 0.447 (−0.001), exact-table match 0.000, truncation 0.038; `adapted_beats_frozen` is true because that flag uses cell accuracy, not because every metric improved. Drawn-chart cell accuracy remained 0.9. Adapter 29 tensors / 75,522,608 bytes, fresh reload parity 8/8 identical tables. Preserved SHA-256: result `83c0e8feeb78…`, evaluation report `d453497d4a2c…`, input manifest `3f74fd5ca99d…`, table CSV `be3928cd4bdf…`, training JSONL `b9c54d2477fe…`, adapter `c04d63c37554…`, adapter manifest `e3286167b3f7…` | 4983.1 s (pass 1 189.3 s stopped at the install cell with the expected stale-module guard; kernel restarted; pass 2 4793.7 s) | **PASSED** — 11/11 post-restart code cells with zero error outputs; promotion evidence for this exact blob. One seeded split of one shard on one runtime, no dispersion estimate; not a DePlot benchmark. |
...
**Release-grade** for source revision `ddf0cc4` / notebook blob `90d9093cced4`, committed at `dd5724e`. The carried
split contract groups records by both declared image identity and SHA-256 of the image bytes, and adaptation rollback
covers epoch-0 validation and progress callbacks. The exact blob passed the fresh Kaggle Tesla T4 v4 run above with
11/11 post-restart code cells and zero error outputs. The passing `efb92ad` / `fb192936d3e6` v3 run remains historical
evidence only for that prior blob.
