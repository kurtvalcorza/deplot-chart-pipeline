...
from deplot_chart_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
    DePlotPipeline,
...
_CONFIG = json.dumps({"model_type": "pix2struct", "is_vqa": False}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        DePlotPipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
